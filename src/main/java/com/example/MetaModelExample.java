...
public class MetaModelExample {
  public static void main(String args[]) {
    ...
    String csvFile =
        ClassLoader.getSystemClassLoader().getResource("example.csv").getPath();
    // String xlsxFile = ClassLoader.getSystemClassLoader()
    //                       .getResource("example.xlsx")
    //                       .getPath();
    // String jsonFile = ClassLoader.getSystemClassLoader()
  ...
  //       DataContextFactory.createExcelDataContext(new File(filename));
  ...
  //     List<Column> sheetColumns =
  //         dataContext.getDefaultSchema().getTableByName(sheetName).getColumns();

  //     for (Column col : sheetColumns)
  //       System.out.println(col.getName());

  //     DataSet content =
  //         dataContext.query().from(sheetName).selectAll().execute();
  ...
  //   DataContext dataContext =
  //       DataContextFactory.createJsonDataContext(new File(filename));
  ...
  //   UpdateableDataContext dataContext =
  //       DataContextFactory.createMongoDbDataContext("localhost", 27017,
  //       "test",
  //                                                   null, null);
  ...
  //         callback
  //             .createTable(callback.getDataContext().getDefaultSchema(),
  //             table) .withColumn("color") .ofType(ColumnType.VARCHAR)
  //             .withColumn("size")
  //             .ofType(ColumnType.CHAR)
  //             .execute();
